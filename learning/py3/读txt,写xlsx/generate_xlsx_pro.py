...
def get_invoice_other_info(order_no_order_id_dict):
    ...
    with DB(host='order-mysql-master', port='3306', user='ebs_operate',
        ...
        if len(order_no_order_id_dict) > 0:
            for order_delivery_no, order_id in order_no_order_id_dict.items():
                sql = "select order_no,create_time,province,city,area,receiver_name from sdb_order_invoice where order_id = '%s'" % order_id
...
if __name__ == '__main__':
    ...
    sheet = 'Sheet1'
    value_list = []
    xlsx_title = ["订单id", "订单号", "开票单号", "发票号", "用户名", "发票抬头", "纳税人识别号", "发票申请时间", "开票金额", "收货人姓名", "收货人手机号", "收货省份", "收货城市",
                  "收货区域", "详细地址"]
