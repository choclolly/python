...
def send_message():
    ...
    connection = pika.BlockingConnection(pika.ConnectionParameters(host='mall_mq_slb', port=5672,
                                                                   credentials=pika.PlainCredentials('admin',
                                                                                                     'Dayl@%L7r')))
    # 创建一个 AMQP 信道（Channel）,建造一个大邮箱，隶属于这家邮局的邮箱
    channel = connection.channel()
    # 声明消息队列tester，消息将在这个队列传递，如不存在，则创建
    channel.queue_declare(queue='order_paper_invoice_queue', durable=True)
    failNum = succNum = 0
    array = []
    with open('3-专票.txt', 'r', encoding='utf-8') as lines:
    ...
    for message in array:
        try:
            channel.basic_publish(exchange='order.notify.exchange', routing_key='order.paper.invoice.queue',
