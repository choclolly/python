...
if __name__ == '__main__':
    # 用户中心库
    with DB(host="172.17.186.153", port="3306", user="test_tea_uc", passwd="pw@kbm9p0Yl@!t3bZD",
        ...
        if len(result) != 0:
            ...
            for x in result:
                ...
                i = int(str(user_id)) % 2
                if i == 0:
                    with DB(host="172.17.186.153", port="3306", user="test_tea_uc_0",
                            passwd="L~+SJ*F^kon[t+10l6", database="test_tea_uc_0") as db_user_0:
                        db_user_0.execute(sql_3, None)
                        db_user_0.execute(sql_4, None)
                        db_user_0.execute(sql_5, None)
                if i == 1:
                    with DB(host="172.17.186.153", port="3306", user="test_tea_uc_1",
                            passwd="]J1tFUzKh8oI#LU9ih",
                            database="test_tea_uc_1") as db_user_1:
