"""
    参考资料:
        Python3.6实现12306火车票自动抢票
            url
                https://cloud.tencent.com/developer/news/233268

                https://segmentfault.com/a/1190000015136244

    Python3.7实现12306火车票自动抢票
    1，输入抢票基本信息
    2，进入网页，手动输入验证码，并点击登录操作
    3，登陆后，自动进入到抢票页面
    4，坐等刷票结果就好了
    ---
    通过splinter刷12306火车票
                    ...
                    if current_tr.find_by_tag('td')[self.seat_type_index].text == '--':
                        ...
                        for p in self.passengers:
                            ...
                            if self.seat_type_value != 0:
                                ...
                                seat_select.find_by_xpath("//option[@value='" + str(self.seat_type_value) + "']")[
                                    0].click()
                            key_value += 1
                            sleep(0.5)
                            if p[-1] == ')':
...
if __name__ == '__main__':
    ...
    # 开始抢票
    ticket = BrushTicket(user_name, password, passengers, from_time, from_station, to_station, number, seat_type,
                         receiver_mobile, receiver_email)
    ticket.start_brush()
