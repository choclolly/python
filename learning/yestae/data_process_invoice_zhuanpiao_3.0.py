...
def get_orderId_by_sku(product_no):
    ...
    with DB(host='order-mysql-master', port='3306', user='ebs_operate',
        ...
        # 仓颉号已发货的订单信息
        # SELECT member_id,member_name,member_no,member_phone,id FROM sdb_order WHERE id in (SELECT order_id FROM sdb_order_goods WHERE product_no = 'DB000227PDH') AND deliver_state = 1 AND order_state=3
        sql = "SELECT member_id,member_name,member_no,member_phone,id FROM sdb_order WHERE id in (SELECT order_id FROM sdb_order_goods WHERE product_no = '%s') AND deliver_state = 1 AND order_state in (3,4)" % product_no
        db_order.execute(sql, None)
        result = db_order.fetchall()
        if result:
            for x in result:
                order_id = x[4]  # .decode('utf-8')
                member_info_dict = {'member_id': x[0], 'member_name': x[1], 'member_no': str(x[2]), 'member_phone': x[3]}
    ...
    with DB(host='order-mysql-master', port='3306', user='ebs_operate',
        ...
        if len(cangjie_order_id_list) > 0:
            for order_id in cangjie_order_id_list:
                ...
                sql = "SELECT invoice_type,invoice_content,receiver_phone FROM sdb_order_invoice WHERE order_id = '%s' and invoice_type = '5'" % order_id
                db_order.execute(sql, None)
        ...
        print('申请了专票的仓颉号订单数量:{}'.format(len(apply_order_invoice_order_id)))
...
def get_order_invoice_info(order_id_order_delivery_no_dict):
    ...
    with DB(host='mall-mysql-master', port='3306', user='invoice_prod',
            passwd='qB0ZCl!ZlJJs^0o',
            database='invoice_flow_mgr') as db_order_conn:
        db_order = db_order_conn.cursor()
        # print('db_order_0:{}'.format(db_order))
        order_ids = order_id_order_delivery_no_dict.keys()
                    ...
                    if result is None:
                        ...
                        for invoice_type, invoice_content in apply_order_invoice_order_id_invoice_type_content_dict[
                            ...
                            if invoice_type == '5':  # 专票
                                ...
                                invoiceType = '5'
                                custAddr = invoice_content['specialCompanyAddr']
                                specialCompanyBank = invoice_content['specialCompanyBank']
                                specialBankAccount = invoice_content['specialBankAccount']
                        # 用户信息
                        ...
                        body_list = []
                        body = {}
                        body['custAddr'] = re.sub(filter_special_characters, '', custAddr)
                        body['custBankAccount'] = re.sub(filter_special_characters, '', specialCompanyBank) + re.sub(
                            filter_special_characters, '', specialBankAccount)
                        body['custName'] = re.sub(filter_special_characters, '', custName)
                        body['custPhone'] = re.sub(filter_special_characters, '', custTelephone)
                        body['custTaxNo'] = custTaxNo
                        # body['custTelephone'] = custTelephone
                        ...
                        body['receiverAddress'] = re.sub(filter_special_characters, '', custAddr)
                        body['receiverPhone'] = custTelephone

                        # 消息队列
                        exchange = 'order.notify.exchange'
                        routing_key = 'order.paper.invoice.queue'
                        queue = 'order_paper_invoice_queue'
                        body_list.append(body)
                        body = json.dumps(body_list, ensure_ascii=False)
                        # print('mq消息体body:\n{}'.format(body))
                        print('订单id:{},物流单号:{},电票mq消息体body:\n{}'.format(order_id, order_delivery_no, body))
                        canjie_zhuanpiao_mq_msg_list.append(body)
...
def get_goods_properties_by_goodsSku(goodsSku, order_delivery_no):
    ...
    with DB(host='dev-sharding-m.tae-tea.net', port='3306', user='root',
        ...
        if result:
            ...
            goodSku_product_id_dict['name'] = result[0]
...
if __name__ == '__main__':
    ...
    with open('canjie_zhuanpiao_mq_msg_{}.csv'.format(time.strftime('%Y%m%d%H%M%S', time.localtime())), 'w',
              newline='') as f:
        for x in canjie_zhuanpiao_mq_msg_list:
